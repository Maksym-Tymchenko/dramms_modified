...
../deedsBCV/linearBCV -M $output_folder/Case${case_num}-US.nii.gz \
...
../deedsBCV/applyBCV -M $output_folder/Case${case_num}-US-landmarks.nii.gz \
-O $output_folder/zero \
-D $output_folder/Case${case_num}-US-landmarks_rotated.nii.gz \
-A $output_folder/affine_matrix.txt

 ../deedsBCV/applyBCV -M $output_folder/Case${case_num}-US.nii.gz \
 -O $output_folder/zero \
 -D $output_folder/Case${case_num}-US_to_MRI_rigid.nii.gz \
 -A $output_folder/affine_matrix.txt


# Perform multimodal registartion using -w 1 option (CC coefficient) important for multimodal registration
dramms -S $output_folder/Case${case_num}-US_to_MRI_rigid.nii.gz\
 -T $output_folder/Case${case_num}-MRI_in_US.nii.gz\
 -O $output_folder/Case${case_num}-US_to_MRI_deformed_result.nii.gz\
 -D $output_folder/Case${case_num}-US_to_MRI_def.nii.gz\
 -L $output_folder/Case${case_num}-US-landmarks_rotated.nii.gz\
 -W $output_folder/Case${case_num}_deformed_seg_US_to_MRI.nii.gz\
 -r 0\
...
python3 ./landmarks_centre_mass.py --inputnii $output_folder/Case${case_num}-MRI-landmarks.nii.gz \
--movingnii $output_folder/Case${case_num}_deformed_seg_US_to_MRI.nii.gz \
--savetxt $output_folder/Case${case_num}-results
